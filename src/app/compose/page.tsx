...
export default function Compose() {
  // passphrase gate (client-side)
  const [key, setKey] = useState<string>(""); // what you type in the UI
        ...
        <>
          <section className="p-5 border rounded-2xl grid gap-4 md:grid-cols-2">
            <label className="grid gap-2">
              ...
              <input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="h-11 rounded-xl border px-3" />
            </label>
            ...
            <label className="grid gap-2">
              ...
              <select
                value={part}
                onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setPart(e.target.value as "am" | "pm")}
            ...
            <label className="grid gap-2">
              ...
              <select value={phase} onChange={(e) => setPhase(e.target.value as Phase)} className="h-11 rounded-xl border px-3">
            ...
            <label className="md:col-span-2 grid gap-2">
              ...
              <input value={tags} onChange={(e) => setTags(e.target.value)} className="h-11 rounded-xl border px-3" placeholder="herbal, sleep, TCM" />
            </label>

            <label className="md:col-span-2 grid gap-2">
              <span className="text-xs uppercase opacity-70">Symptoms (comma)</span>
              <input value={symptoms} onChange={(e) => setSymptoms(e.target.value)} className="h-11 rounded-xl border px-3" />
            </label>

            <label className="md:col-span-2 grid gap-2">
              <span className="text-xs uppercase opacity-70">Practices (comma)</span>
              <input value={practices} onChange={(e) => setPractices(e.target.value)} className="h-11 rounded-xl border px-3" />
            </label>
            ...
            <label className="grid gap-2">
              ...
              <input value={mood} onChange={(e) => setMood(e.target.value)} className="h-11 rounded-xl border px-3" />
            </label>

            {/* Optional digestion fields (not yet saved to Entry) */}
            <label className="md:col-span-2 grid gap-2">
              <span className="text-xs uppercase opacity-70">Digestion (free text)</span>
              <textarea value={digestionNotes} onChange={(e) => setDigestionNotes(e.target.value)} className="min-h-[100px] rounded-xl border p-3" />
            </label>

            <label className="md:col-span-2 grid gap-2">
              <span className="text-xs uppercase opacity-70">Digestion keywords (comma)</span>
              <input value={digestionTags} onChange={(e) => setDigestionTags(e.target.value)} className="h-11 rounded-xl border px-3" />
            </label>
              ...
              <div className="flex gap-3">
                {(["public","anon","private"] as Privacy[]).map(p => (
                  <label key={p} className="flex items-center gap-2 text-sm">
                    <input type="radio" name="privacy" value={p} checked={privacy===p} onChange={() => setPrivacy(p)} /> {p}
                  </label>
                ))}
              </div>
            </fieldset>
            ...
            <label className="md:col-span-2 grid gap-2">
              ...
              <textarea value={notes} onChange={(e) => setNotes(e.target.value)} className="min-h-[140px] rounded-xl border p-3" placeholder="Free-form notes…" />
            </label>
          </section>
          ...
          <section className="p-5 border rounded-2xl space-y-3">
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <button onClick={() => navigator.clipboard.writeText(json)} className="inline-flex h-10 items-center rounded-full border px-4 text-sm hover:bg-neutral-800">Copy</button>
                <button onClick={() => download(json, `${id || "entry"}.json`)} className="inline-flex h-10 items-center rounded-full border px-4 text-sm hover:bg-neutral-800" disabled={!id}>Download</button>
                <button onClick={addToLocal} className="inline-flex h-10 items-center rounded-full border px-4 text-sm hover:bg-neutral-800" disabled={!id}>Add to local drafts</button>
                <button onClick={saveToSite} className="inline-flex h-10 items-center rounded-full border px-4 text-sm hover:bg-neutral-800" disabled={!id || !allowed}>Save to site</button>
              </div>
            </div>
            <pre className="text-xs overflow-x-auto rounded-lg p-3 bg-neutral-900/40 border">{json}</pre>
            <p className="text-xs opacity-70">Local drafts merge into the library for preview; commit to JSON when ready.</p>
          </section>
        </>
      )}
    </main>
