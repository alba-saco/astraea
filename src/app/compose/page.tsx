...
export default function Compose() {
  ...
  const entry = {
    ...
    herbs: [], // optional, placeholder
    threads: threads.split(",").map(s => s.trim()).filter(Boolean), // <-- NEW
    mood: mood || null,
    privacy,
    notes,
    schema_version: 1
  ...
  const addToLocal = () => {
    const curr = JSON.parse(localStorage.getItem("astraea_drafts") || "[]");
      ...
      {process.env.NEXT_PUBLIC_COMPOSE_KEY && !allowed ? (
        <div className="p-4 border rounded-xl space-y-3">
          ...
          <input type="password" value={key} onChange={(e)=>setKey(e.target.value)}
                 placeholder="Passphrase" className="h-11 w-full rounded-xl border px-3" />
        </div>
      ) : (
        <>
          <section className="p-5 border rounded-2xl grid gap-4 md:grid-cols-2">
            ...
            <label className="grid gap-2">
              ...
              <input type="number" min={1} max={30}
                     value={cycleDay} onChange={(e)=>setCycleDay(e.target.value===""?"":Number(e.target.value))}
                     placeholder="e.g. 13" className="h-11 rounded-xl border px-3"/>
            </label>
            <label className="grid gap-2">
            ...
            <label className="md:col-span-2 grid gap-2">
              ...
              <input value={tags} onChange={(e)=>setTags(e.target.value)}
                     className="h-11 rounded-xl border px-3" placeholder="herbal, sleep, TCM"/>
            </label>
            <label className="md:col-span-2 grid gap-2">
            ...
            <label className="md:col-span-2 grid gap-2">
              ...
              <input value={practices} onChange={(e)=>setPractices(e.target.value)}
                     className="h-11 rounded-xl border px-3" />
            </label>
            <label className="md:col-span-2 grid gap-2">
                ...
                {(["public","anon","private"] as Privacy[]).map(p=>(
                  <label key={p} className="flex items-center gap-2 text-sm">
                    <input type="radio" name="privacy" value={p}
                      checked={privacy===p} onChange={()=>setPrivacy(p)}/>
                    {p}
                  </label>
                ))}
              </div>
            ...
            <label className="md:col-span-2 grid gap-2">
              ...
              <textarea value={notes} onChange={(e)=>setNotes(e.target.value)}
                        className="min-h-[140px] rounded-xl border p-3" placeholder="Free-form notes…"/>
            </label>
          </section>
          ...
          <section className="p-5 border rounded-2xl space-y-3">
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={()=>download(json, `${id||"entry"}.json`)}
                        className="inline-flex h-10 items-center rounded-full border px-4 text-sm hover:bg-neutral-800" disabled={!id}>
                  Download
                </button>
